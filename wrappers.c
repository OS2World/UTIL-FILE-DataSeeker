...
Usage subject to 'Modified BSD License' as described in COPYING. Some portions
of code subject to the Sybase Open Watcom Public License version 1.0. See
SYBASE_OW_LICENSE. These portions are explicitly marked.
*******************************************************************************/
...
// in pmseek.h  #define INCL_DOS
// in pmseek.h  #define INCL_LONGLONG
// in pmseek.h  #define INCL_DOSFILEMGR   /* File Manager values */
// in pmseek.h  #define INCL_DOSERRORS    /* DOS error values */
// in pmseek.h  #define INCL_DOSMODULEMGR
// in pmseek.h  #define INCL_WINDIALOGS     // needed for sw.h
#include "pmseek.h"

// in pmseek.h  #include <os2.h>
// in pmseek.h  #include <stdio.h>
// in pmseek.h  #include <string.h>
...
//#include "wrappers.h" in pmseek.h
  ...
  if ( !iLargeFileSupport )
    ...
    switch ( ulInfoLevel )
      ...
      case FIL_QUERYEASIZEL:
        ...
        break;
      default:
        TRACE1("ulInfoLevel=%u is unexpected", ulInfoLevel);
  ...
  if ( !iLargeFileSupport )
    ...
    switch ( ulLastInfoLevel )
      ...
      case FIL_QUERYEASIZEL:
        ...
        break;
      default:
        TRACE1("ulLastInfoLevel=%u is unexpected", ulLastInfoLevel);
        rc = ERROR_INVALID_PARAMETER;
      } // switch
    }
  else
    rc = DosFindNext(hDir, pfindbuf, cbfindbuf, pcFileNames);
  //TRACE2("rc=%d, size: %lld", rc, ((PFILEFINDBUF3L)pfindbuf)->cbFile );
  return rc;
  }
 ...
 */
// 20090615 LFS
APIRET xDosQueryFileInfo (HFILE hf, ULONG ulInfoLevel, PVOID pInfoBuf, ULONG cbInfoBuf)
  {
  FILESTATUS3 fs3;
  FILESTATUS4 fs4;
  APIRET rc;
...
LONGLONG xlseek(    int iHandle,
  ...
  switch ( iOrigin )
    ...
    case SEEK_END:
      ...
      break;
    default:
      TRACE1("(currently) not supported origin %d", iOrigin);
      errno = EINVAL;
      return( -1 );
    }
  ...
  if ( rc != 0 )
    {
    TRACE1("xDosSetFilePtr returned error %d", rc);
    switch ( rc )
      {
...
static int __sopen( const char *name, int mode, int share, va_list args )
  ...
  error = xDosOpen( (PSZ) name,
                    ...
                    openmode,
                    0L );
  TRACE3("error=0x%X, handle=0x%X, '%s'", error, handle, name );
  if ( error )
    {
    _doserrno = error;
    return( error );
  ...
  /* no check necessary, DosOpen did it for us here
      if( handle >= __NFiles ) {
          DosClose( handle );
          _doserrno = ENOMEM;
          return( -1 );
  ...
  */
  ...
  if ( mode & (O_BINARY|O_TEXT) )
    ...
    {
    // ToDo: _RWD_fmode is default file mode in OW  if( _RWD_fmode == O_BINARY )    iomode_flags |= _BINARY;
    }
  if ( isatty( handle ) )              iomode_flags |= _ISTTY;

  //    if( iomode_flags & _BINARY )        _set_binary( handle );    // does nothing

  //ToDo: __SetIOMode( handle, iomode_flags );
